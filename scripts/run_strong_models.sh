...
# Test and advanced-user escape hatch: run with an already prepared Python.
if [[ -n "${ACWORLD_RUNNER_PYTHON:-}" ]]; then
  export PYTHONPATH="$repo_root/src${PYTHONPATH:+:$PYTHONPATH}"
  exec "$ACWORLD_RUNNER_PYTHON" -m cli.strong_models "$@"
fi
...
# Bootstrap uv inside the repository when the machine has only Python 3.
...
exec "$uv_command" run --frozen acworld-strong-models "$@"
